...
Python solution code (if any) to a .py file. Output filenames are derived,
not read from the links file. Files are written to the current directory.

Usage: generate-neetcode-solutions.mjs <links.txt>
...
if (!inputFile) {
  console.error("Usage: node generate-neetcode-solutions.mjs <links-file>");
  process.exit(1);
}
...
async function main() {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      const solution = data.solutions.python;
      const filename = `${number}.${processName(data.name)}_solution.py`;
      fs.writeFileSync(filename, solution, "utf8");
    } catch (err) {
