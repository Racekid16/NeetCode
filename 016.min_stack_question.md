...
Ouput: [null, null, null, null, 0, null, 2, 1]
```
...
<details>
  ...
  A brute force solution would be to always check for the minimum element in the stack for ``getMin()`` function call. This would be ``O(n)`` approach. Can you think of a better way? Maybe ``O(n)`` extra space to store some information.
</details>
